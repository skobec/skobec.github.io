...
@since 2013-04-14 <- 2013-12-23 <- 2013-09-25 <- 2013-09-13
    ...
    addMoreFileFieldEx: function(target, name, max_input_count) {
        ...
        $(inp).on('change', function(e){
            ...
            if(target) {
				...
				CustomUI.addMoreFileFieldEx(target, name, max_input_count);
			}
			return false;
        });
        inp.styler({
    ...
    */
    ...
    showWait: function(text, caption, wait_url) {
        ...
        //if(CustomUI.popup_form_opened_forms_id.length < 1) {
            ...
            return CustomUI.showMessage('<div style="text-align: center;" id="popup-form-wait_message">' + this.wait_image + '<br />' + text + '</div>', caption, 'info', {wait_url: wait_url});
        //}
    }
}
                                    ...
                                    if(options.json) {
                                        ...
                                        try {
                                            	...
                                            	if(obj.error_field_list instanceof Object) {
													...
													$.each(obj.error_field_list, function(key, value) {
														var field = $('#' + options.target_form).find('[name="form[' + key + ']"]')
															...
															.validationEngine('showPrompt', value ? value : 'Необходимо заполнить', 'error', false, false);
													});
                                            		return;
												} else {
                                                	throw obj.message;
                                            ...
                                            if(options.error instanceof Function) {
                                                ...
                                                // вывод информирующей формы
                                                if((typeof data.code != 'undefined') && (data.code == 211)) {
                                                    CustomUI.hideForm('');
                                                    return CustomUI.showForm('form-login');
                                                }
