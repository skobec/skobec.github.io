...
/**
* Генератор Штрих-код 128
* @author Ленар Закиров (Эттон)
* @since 2014-08-12
*/
class Barcode_128 {
	static function encode($chaine) {
		$chaine = mb_convert_encoding($chaine, 'ascii', 'utf-8');
		$code128 = "";
		if(strlen($chaine) > 0){
			$z = 0;
			$i = 1;
			...
			if($i > 0){
				...
				while($i<=strlen($chaine)){
					if($tableB) {
						...
						$mini=self::Testnum($mini,$chaine,$i);
						if($mini < 0) {
							if($i == 1){
								$code128 = chr(205);
							}else{
					...
					if(!$tableB){
						...
						$mini=self::Testnum($mini,$chaine,$i);
						if($mini < 0){
							$dummy=self::Myval(substr($chaine, $i-1, 2));
							if($dummy < 95){
								$dummy = $dummy + 32;
							} else {
								$dummy = $dummy + 100;
		...
		$code128 = mb_convert_encoding($code128, 'utf-8', 'ascii');
		return htmlentities($code128);
	}

	static function Testnum($mini,$chaine,$i){
		$mini=$mini-1;
