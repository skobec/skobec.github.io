...
class Paginator extends Type_Paginator {

	const PAGINATOR_IPP_SELECT = 1;
	const PAGINATOR_IPP_LINK = 2;
	const PAGINATOR_STYLE_SCROLL = 1;
	const PAGINATOR_STYLE_STATIC = 2;

	private $id = null;
	private $start_index = 0;
	private $show_ipp_choose = false;
	private $ipp_array;
	...
	public function __construct($id, $items_per_page = Constant::DEF_ITEMS_PER_PAGE, $total_pages = null, $current_page = null, $show_ipp_choose = false) {
		$this->id = $id;
		...
		$this->fixRange();
		// Показывать выбиратор количества страниц
		if(is_array($show_ipp_choose)) {
			$this->show_ipp_choose = true;
			$this->ipp_array = $show_ipp_choose;			
		} else {
			$this->show_ipp_choose = true; // $show_ipp_choose;
			$this->ipp_array = array(Constant::DEF_ITEMS_PER_PAGE, 50, 100);
		}
	}
	...
	*/
	public function getCalculated($db, $select) {
		$select
		->reset(Zend_Db_Select::ORDER)
		/*->reset(Zend_Db_Select::COLUMNS)*/
		->reset(Zend_Db_Select::LIMIT_COUNT)
		->reset(Zend_Db_Select::LIMIT_OFFSET);
		$secondQuery = $db->select()->from(array('s' => $select), array('cnt' => 'COUNT(*)'));
		// file_put_contents(dirname(__FILE__).'/'.rand(0, 88888888).'txt', trim($secondQuery));
		// dumpre(trim($secondQuery));
		$r = $secondQuery->assemble();
		$recordsCount = (array)$db->fetchRow($r);            
		$this->records_count = (int)$recordsCount['cnt'];
		$this->total_pages = ceil($this->records_count / $this->items_per_page);
		return Functions::cast($this, 'Type_Paginator');
	}
           
    /**
	* Генерация пагинатора для api-методов по sql
	* 
        * @param object $db	
	* @param string $sql	
	* 
	* @return Type_Paginator
	*/
	public function getCalculatedManual($db,$sql) {
                $arr=explode ("limit",$sql);
                $sql= array_shift($arr);
                $sql="select count(*) as cnt from (".$sql.")qq";
                $stmt = $db->query($sql); 
                $arr = $stmt->fetchAll();
                $arr=array_shift($arr);
                $this->records_count= (int)$arr->cnt;
		$this->total_pages = ceil($this->records_count / $this->items_per_page);
                return Functions::cast($this, 'Type_Paginator');
	}           
	...
	*/
	public function __toString() {
		...
		try {
			if(Zend_Layout::getMvcInstance()) {
				...
				if($view) {
					$view->headLink()->appendStylesheet('/paginator/paginator3000.css');
					$view->headScript()->appendFile('/paginator/paginator3000.js');
				}
			}
			if($this->total_pages > 1) {
				...
				if($this->getPaginationStyle() == self::PAGINATOR_STYLE_SCROLL) {
					...
					<script type="text/javascript">
						pag1 = new Paginator('paginator_<?=$this->id?>', <?= $this->total_pages?>, 10, <?= $this->current_page?>, '<?= urldecode($view->url(array())).$this->getQueryString() ?>');
					</script>
					<?php
				} elseif ($this->getPaginationStyle() == self::PAGINATOR_STYLE_STATIC) {
					$visible_count = 8;
					$url = urldecode($view->url(array())).$this->getQueryString();
			...
			if(($this->records_count > Constant::DEF_ITEMS_PER_PAGE) && $this->show_ipp_choose) {
				if($this->getIppStyle() == self::PAGINATOR_IPP_SELECT) {
					$option_list = '';
					foreach($this->ipp_array as $ipp) {
	...
	private function getUrlWithIpp($count) {
		$getParams = Zend_Controller_Front::getInstance()->getRequest()->getQuery();
		$key = $this->id.'_count';
		$getParams[$key] = $count;
		$base_url = Zend_Controller_Front::getInstance()->getRequest()->getPathInfo();
		return rtrim($base_url, '/').$this->getQueryString($getParams).'1';
	}
	...
	public static function draw($paginator = null) {
		if(!is_object($paginator)) {
			return false;
		}
		if(!($paginator instanceof Paginator)) {
			$paginator = unserialize(str_replace('O:8:"stdClass"' ,'O:9:"Paginator"', serialize($paginator)));
		}
		echo trim($paginator);
	}

    public static function drawCounter($paginator = null){
        if(!is_object($paginator)) {
            return false;
        }
