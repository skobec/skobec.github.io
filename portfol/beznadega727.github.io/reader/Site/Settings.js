...
function setCustomColor(clicked) {
...
function patchViewportHeight(h) {
    ...
    if (pda.state && win.screen) {
        ...
        if (pda.platform == 'ios' ||
            ...
            if (!pda.portrait && (pda.platform == 'ios' || pda.platform == 'winphone')) {
        ...
        if (pda.platform == 'android') {
            if (pda.browser == 'chrome') {
                ...
                if (pda.portrait) {
                    ...
                    if (pda.version.match(/^4\.0/i) != null) {
                        h -= 29;
                    }
                }
                else {
...
function calcSettingsArrowPos() {
    var settingsWin = doc.querySelector('#settings');
    var settinsgButton = doc.querySelector('.menu-settings');
...
function checkPDAstate() {
    ...
    // alert(ua);
    if ((ua.match(/Windows NT/i) != null && ua.match(/Touch/i) != null) ||
