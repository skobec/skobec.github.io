var DragManager = new function() {
  ...
  function findDroppable(e) {
	...
	var item_id = dragObject.avatar.id;
	var index = 0;
	var res = elem.closest('.droppable.'+item_id);
	if(res) {
		index = $('#result_line td').index(res);
		if(index == 0) {
			return res;
		}else{
			if($('#result_line .droppable.'+item_id).prev().hasClass('selected')) {
				return res;
			}
		}
	}
