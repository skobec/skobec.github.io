...
ChartControl.prototype.convertToMonthsState = function () {
    var fact = [];
    var plan = [],
        filter_years_length = this.opts.filter_years.length,
        other_data_length = this.master_data_other.length;
    for (var item = 0; item < other_data_length; item++) {
        var fact_data = [];
        var plan_data = [];
        var xAxis = [];

        for (var y = 0; y < filter_years_length; y++) {
            var year = this.opts.filter_years[y];
            for (var i = 1; i < 13; i++) {
                /* Формирование оси X*/
                if (filter_years_length > 1 && i == 1) {
                    xAxis.push(year);
                } else {
                    xAxis.push(i);
                }
...
ChartControl.prototype.renderCustomPie = function () {
    ...
    return {
        ...
        tooltip: {
            positioner: function (labelWidth, labelHeight, point) {
                var coords = {x: point.plotX - 55, y: point.plotY - 200};
                return coords;
            },
            shape: 'square',
            useHTML: true,
...
ChartControl.prototype.compareData = function () {
    var self = this, other_plan_length, new_data, item, i;
    if (self.opts.filter_compare) {
        new_data = [];
        other_plan_length = self.other_plan.length;
        //если выбран фильтр план/факт объединяем данные плана и факта в один массив , факт данные будут представлены в виде линий
        for (i = 0; i < other_plan_length; i++) {
            new_data.push(self.other_plan[i]);

            item = clone(self.other_fact[i]);//клонирование объекта факт, иначе изменяется оригинал
            item.type = 'spline';
            item.color = colors[i];
        ...
        mouseenter: function () {
            ...
            waiter = setTimeout(function () {
                ...
                switch (self.chart_type) {
                    case 'pie':
                        var series = self.opts.chart.highcharts().series[0].data[index];
                        series.setState('hover');
                        self.opts.chart.highcharts().tooltip.refresh(series);
                        break;
                    case 'columns':
        ...
        mouseleave: function () {
            ...
            switch (self.chart_type) {
                case 'pie':
                    ...
                    self.opts.chart.highcharts().tooltip.hide();
                    break;
                case 'columns':
                    var data = self.opts.chart.highcharts().series[index].data;
...
ChartControl.prototype.sortChart = function () {
    var other_data = this.other_data, other_data_length;
    var new_colors = [],i;
    ...
    other_data_length = other_data.length;
    for(i = 0;i<other_data_length;i++){
...
ChartControl.prototype.setDataVisiblity = function () {
    var i, data_length;
    data_length = this.other_data.length;
...
// $(function(){
//     $("#widgets_container").on("click","a", function (event) {
//         event.preventDefault();
//         var id  = $(this).attr('href'),
//             top = $(id).offset().top;
//         $('.dashboard').animate({scrollTop: top}, 500);
//     });
// });
        $(function(){
